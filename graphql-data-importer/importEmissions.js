...
async function addEmissionsDatabaseRows(auth) {
  ...
  //   );
    ...
    const { blockNumber, timestamp, runDateUTC } = await getBlockForCurrentDate(0);
    ...
    if (!isTimestampInSheet) {
      const emissions = await getEmissionsData(blockNumber, 0);
    ...
    if (!isTimestampInSheet) {
      const tokens = await getTokenData(blockNumber, 0);
    ...
    if (!isTimestampInSheet) {
      const beetsPerBlock = await getBeetsPerBlock(blockNumber, 0);
